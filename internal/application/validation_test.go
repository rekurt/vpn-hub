	...
	tests := map[string]struct {
		...
		"client key that is not base64": {
			func(c *domain.Config) {
				c.Devices[0].Profiles[0].ClientPrivateKey = ""
				c.Devices[0].Profiles[0].ClientPublicKey = "nope!"
			}, "not valid base64",
		},
		...
		"profile address outside the client subnet": {
			func(c *domain.Config) { c.Devices[0].Profiles[0].Address = "192.168.1.5/32" }, "outside hub client_cidr",
		},
		"profile address that is not a host route": {
			func(c *domain.Config) { c.Devices[0].Profiles[0].Address = "10.80.0.0/24" }, "must be a host route",
		},
		...
		"tunnel id too long for an interface name": {
			func(c *domain.Config) {
				...
				c.Devices[0].Profiles[0].Egress = "a-very-long-tunnel-id"
			}, "at most 12",
		},
		"duplicate device": {
		...
		"two profiles sharing an address": {
			func(c *domain.Config) {
				second := c.Devices[0].Profiles[0]
				second.ID = "macbook-second"
				c.Devices[0].Profiles = append(c.Devices[0].Profiles, second)
			}, "is shared by",
		},
		"duplicate profile id": {
			func(c *domain.Config) {
				c.Devices[0].Profiles = append(c.Devices[0].Profiles, c.Devices[0].Profiles[0])
			}, "duplicate profile",
		},
		"egress pointing at a private-network tunnel": {
			func(c *domain.Config) { c.Devices[0].Profiles[0].Egress = "corp" }, "is not an egress tunnel",
		},
		"egress that does not exist": {
			func(c *domain.Config) { c.Devices[0].Profiles[0].Egress = "nowhere" }, "is not an egress tunnel",
		},
...
func TestValidateAcceptsDirectEgress(t *testing.T) {
	...
	err := validateWith(t, func(c *domain.Config) {
		c.Devices[0].Profiles[0].Egress = domain.EgressDirect
	})
	if err != nil {
