...
- **Статус**: ревизия, ожидание подтверждения, дрейф хоста, здоровье туннелей, состояние агента.
- **Устройства**: смена egress, добавление с доставкой профиля файлом и QR-кодом, перевыпуск профиля (снимает отзыв), отзыв. Приватный ключ устройства существует только в момент генерации и уходит в чат, на хабе не остаётся.
- **Туннели**: вкл/выкл (с той же проверкой «не оставить устройство без выхода»), проба здоровья, правка маршрутов и DNS-зон.
- **Деплой**: превью изменений → применить со страховкой (сообщение-таймер с кнопками «Подтвердить»/«Откатить»; не подтвердили — агент откатывает сам) или без.
- **Подписки**: ручное обновление с живым прогрессом по кандидатам; плановое — бот сам планировщик (см. ниже).
- **Маршруты, логи юнитов, хост-метрики, настройки уведомлений.**

Все опасные действия — в два нажатия. Все мутации сериализуются через один внутренний gate: canary-namespace один на всех, и одновременные «обновить подписку» и «деплой» не должны толкаться. Пока бот работает, таймер `vpn-hub-subscription@` не нужен и отключается деплоем — плановые обновления идут через тот же gate.
