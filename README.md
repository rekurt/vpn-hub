...
Персональный egress работает и здесь: каждое устройство аутентифицируется собой, его соединения помечаются fwmark его туннеля (номера берутся из того же firewall-плана). Kill switch на этом пути — явные drop-правила в цепочке output: меченый пакет, уходящий не в свой интерфейс, отбрасывается. Это не формальность — `ip rule fwmark N lookup N` при пустой таблице проваливается в main, то есть без этих правил трафик устройства с лежащим туннелем ушёл бы через голый аплинк хаба. Слушатель поднимает сам агент как transient-юнит: конфиг с ключом лежит в tmpfs, переживает перезагрузку и останавливается, когда fallback выключают.
