...
- **Запасные входы на 443 не проверены на живом хосте.** Handshake REALITY теперь покрыт интеграционным тестом (клиент и сервер sing-box, настоящее рукопожатие на пиновой версии), но полный путь — импорт ссылки в мобильный клиент, персональный egress, реальная сеть с зарезанным UDP — на стенде ещё не прогонялся. До этого прогона считать эту часть непроверенной.
- **Трафик запасного входа по TCP/443 не покрыт kill switch.** Kill switch — это политика drop в цепочке forward, а прокси-путь идёт через сокеты самого хаба (input/output), не через forward. Если туннель устройства лёг, его соединения по этому пути не «оборвутся молча», а упрутся в отсутствие маршрута — но защиты уровня forward-политики здесь нет.
...
Персональный egress работает и здесь: каждое устройство аутентифицируется собой, его соединения помечаются fwmark его туннеля (номера берутся из того же firewall-плана), и приватные зоны по-прежнему перебивают выбранный egress. Слушатель поднимает сам агент как transient-юнит — конфиг с ключом лежит в tmpfs, переживает перезагрузку и останавливается, когда fallback выключают.
