...
`hubctl keygen` writes a private hub key with mode `0600` and prints only the public half. `hubctl device add --output …` generates a client key and writes it into the profile with mode `0600`; the CLI does not retain the private key. Losing that profile requires reissue.
